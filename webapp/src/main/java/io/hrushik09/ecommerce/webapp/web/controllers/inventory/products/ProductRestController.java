...
class ProductRestController {
    ...
    @GetMapping("/all")
    List<ProductSummary> getAllProducts(@RequestParam boolean needsRefrigeration) {
        log.info("request to inventory service to get all products");
        int pageNo = 1;
        List<ProductSummary> productSummaries = new ArrayList<>();
        while (true) {
            PagedResult<ProductSummary> pagedResult = inventoryServiceClient.getProductsWithRefrigerationAs(pageNo, needsRefrigeration);
